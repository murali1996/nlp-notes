...
# os.system('wget https://storage.googleapis.com/bert_models/{}/{}.zip -O {}/{}.zip'.format(date,BERT_MODEL_NAME,BERT_PRETRAINED_MODELS_DIR,BERT_MODEL_NAME))
...
class Model(object):
	...
	def save_tokenized_sents(self, *args : "ex: x_tr_inputFeatures, x_tr_raw, y_tr_inputFeatures, y_tr_raw", column_names, file_title, ckpt_dir=None):
		...
		for arg in args:
			...
			if same_len!=len(arg):
				raise Exception("In save_tokenized_sents(), all arguments provided must have same length!!")
...
if __name__=="__main__":
	...
	# each feature object has input_ids, input_mask, segment_ids, label_id, token_len, joined_tokens, is_real_example attributes
